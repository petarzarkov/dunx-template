...
describe('the generated OpenAPI document', () => {
  ...
  /**
   * `page()` is async: the explorer bundle lives behind `@dunx/openapi/swagger`
   * and is reached with `await import()`, so importing `@dunx/openapi` does not
   * pull a UI in with it. 3.7.0 went further and made the renderer an explicit
   * argument - a module with none serves the document and throws here.
   */
