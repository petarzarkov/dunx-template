...
| `CanActivate` guard                                   | a `Middleware` that throws. `SessionGuard` from `@dunx/auth` and `ThrottleGuard` from `@dunx/http`, both listed in `httpOptions.middleware`.                                                              |
...
| `@UseInterceptors(CacheInterceptor)`                  | a `Middleware` over `@dunx/infra/cache`'s `Cache`, with `@NoCache()` to opt a route out. Production only, and the caller is part of the key.                                                              |
...
| `@nestjs/terminus`                                     | `HealthModule` from `@dunx/http`, at `/api/health/live` and `/api/health/ready`. `degraded` turned out to be a vocabulary rather than a capability: `critical: false` reports a check without gating readiness, which is what that bucket meant. What the framework adds is per-indicator timeouts and a drain phase that fails readiness _before_ the socket closes - neither reproducible in app code. |
...
| the Swagger `responseInterceptor` that preauthorizes after sign-in | the same interceptor, passed to `SwaggerRenderer` as the **source of an expression** rather than a function: the page renders on the server, so no closure can travel. `main.ts` has it.                                                                            |
| LinkedIn, Google and GitHub providers                              | the same three, each offered only when both halves of its credential pair are set.                                                                                                                                                                                  |
| the invite flow                                                    | `src/users/invites/`. One `@Public()` handler in an otherwise `@Roles('admin')` controller, which is what guards composing per route rather than per controller buys.                                                                                               |
| `HtmlSessionAuthMiddleware` over the docs and dashboard pages      | one `Authorize` from `@dunx/http`, handed to `DashboardModule` and to `OpenApiModule`, and run over this app's own Scalar mount with `gate()`. It returns a `Response` rather than `false`, which is how the login form survives a contract whose refusal is a 404. |
...
| `@socket.io/redis-emitter` in the worker | `RelayPublisher` from `@dunx/http`, bound by `WsRelayModule` and given the channel the servers listen on. It was two hand-copied wire formats until [dunx#137](https://github.com/petarzarkov/dunx/issues/137) shipped the class in 3.8.2. |

## Cache and rate limiting

| NestJS                                                          | dunx                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        |
| --------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `CacheModule` + `keyv` + `KeyvIoredisAdapter` + `cache-manager` | `@dunx/infra/cache`: an in-process L1 in front of Redis, with metrics, and `DegradingCacheStore` wrapping the L2 so an unreachable backend is a miss rather than a 500. This app wrote that wrapper first and deleted it in 3.9.1, when [dunx#147](https://github.com/petarzarkov/dunx/issues/147) shipped one. It wraps the L2 by hand rather than through `degrade: true`, because `TieredCacheStore.set` awaits L2 before L1 and degrading from outside the tier loses the L1 promotion. |
| `ThrottlerModule` with three named tiers                        | `ThrottleModule` from `@dunx/http`. With no Redis it counts in memory rather than failing open, so the budget is per replica rather than absent.                                                                                                                                                                                                                                                                                                                                            |
...
Two things, and both are the same reason: they are a product's choices, not a
framework's. The list was five. Swagger preauthorization and Postgres both came
...
`@dunx/infra/email` closed the third, in 3.9.0, and closed it the way dunx#139
proposed rather than sideways: a transport seam, a renderer seam, and a
Bun-native preview server. So the mail row moved out of "deliberately not
ported" entirely.

Three more went the same way in 3.9.1, and all three were found by doing this
migration rather than by reading the framework: a cache with no way to degrade
([#147](https://github.com/petarzarkov/dunx/issues/147)), a health report whose
per-check facts could only be prose
([#148](https://github.com/petarzarkov/dunx/issues/148)), and a shutdown drain
that every test teardown paid
([#146](https://github.com/petarzarkov/dunx/issues/146)). Two workarounds in this
repository are gone with them; the third survives in one suite that builds
through `HttpFactory.create`, where the harness cannot reach.

No row is open any more. Both that were - publishing to the websocket relay from
a worker, and gating the explorer the way the dashboard can be gated - closed in
3.8.2 and 3.8.3, which is the note above happening twice inside one week.

Neither closed the way the issue proposed, and that is the more useful half.
dunx#137 asked for two functions to be exported and got `RelayPublisher`, a class
with `PubSub`'s own shape. dunx#140 asked for `authorize` on `OpenApiModule` and
got `Authorize` moved into `@dunx/http`, where both ops surfaces and this app's
own Scalar mount read the same type - so the workaround did not shrink, it went
away.
